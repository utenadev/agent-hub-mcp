...
func (a *App) Run(args []string, stdin io.Reader, stdout io.Writer, stderr io.Writer) error {
	...
	default:
		return fmt.Errorf("Unknown command: %s\nCommands: serve, orchestrator, doctor, setup", command)
	}
}
...
func (a *App) runServe(args []string) error {
	...
	if results, err := database.CheckIntegrity(); err != nil {
		...
		fmt.Fprintf(os.Stderr, "Database check failed:\n")
...
func (a *App) runDoctor(args []string) error {
	...
	if err != nil {
		...
		if err != nil {
			...
			fmt.Println("  [OK] Database integrity check passed")
			fmt.Println("  Table status:")
	...
	fmt.Printf("[*] Checking Database Permissions (%s)... ", *dbPath)
	if _, err := os.Stat(*dbPath); err == nil {
		// Check read permission
		file, err := os.Open(*dbPath)
		if err != nil {
			fmt.Printf("[ERROR] Cannot read database file: %v\n", err)
			allOk = false
		} else {
			file.Close()
			...
			if err != nil {
				fmt.Printf("[ERROR] Cannot write to database file: %v\n", err)
				allOk = false
			} else {
				file.Close()
				fmt.Println("[OK]")
		...
		if err != nil {
			fmt.Printf("[WARN] Database directory not accessible: %v\n", err)
		} else {
			fmt.Printf("[OK] Database will be created in: %s\n", dbDir)
			_ = info
		}
	} else {
		fmt.Printf("[ERROR] Cannot access database path: %v\n", err)
		allOk = false
	}
	...
	if err == nil {
		...
		fmt.Printf("[*] Checking Config Directory (%s)... ", agentHubDir)
		info, err := os.Stat(agentHubDir)
		if err != nil {
			if os.IsNotExist(err) {
				fmt.Println("[WARN] Directory does not exist. Run 'agent-hub setup' to create it.")
			} else {
				fmt.Printf("[ERROR] Cannot access directory: %v\n", err)
				allOk = false
			}
		} else {
			if info.Mode().Perm()&0600 == 0600 {
				...
				fmt.Printf("[WARN] Directory permissions are %v, may cause issues\n", info.Mode().Perm())
			}
		}
	}

	// Check API Keys
	fmt.Print("[*] Checking Gemini API Key... ")
	apiKey := os.Getenv("GEMINI_API_KEY")
	if apiKey == "" {
		apiKey = os.Getenv("HUB_MASTER_API_KEY")
	}
	if apiKey == "" {
		fmt.Println("[WARN] Neither GEMINI_API_KEY nor HUB_MASTER_API_KEY is set. Summarization will be mocked.")
	} else {
		fmt.Printf("[OK] (ends with %s)\n", apiKey[len(apiKey)-4:])
	}

	// Check Agent Config
	fmt.Print("[*] Checking Agent Configuration... ")
	sender := os.Getenv("BBS_AGENT_ID")
	role := os.Getenv("BBS_AGENT_ROLE")
	configOk := true
	if sender == "" {
	...
	if allOk {
		...
		fmt.Println("Run 'agent-hub setup' if this is a fresh installation.")
...
func (a *App) runSetup(args []string) error {
	...
	if _, err := os.Stat(configPath); err == nil && !*force {
		...
		fmt.Printf("[*] Creating Config Template (%s)... ", configPath)
	...
	fmt.Println("Add the following to your Claude Desktop configuration:")
	fmt.Println()
	...
	fmt.Println()
	fmt.Println("JSON snippet to add:")
	fmt.Println("```json")
	fmt.Printf("{\n  \"mcpServers\": {\n    \"agent-hub\": {\n      \"command\": \"%s\",\n      \"args\": [\"serve\", \"-db\", \"%s\"]\n    }\n  }\n}\n", execPath, *dbPath)
	fmt.Println("```")
	fmt.Println()
	fmt.Println("Setup complete! You can now run 'agent-hub serve' or 'agent-hub doctor'.")
